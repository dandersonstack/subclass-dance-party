describe('breakDancer', function() {
  ...
  describe('dance', function() {
    ...
    it('should rotate between the images', function() {
      sinon.spy(breakDancer, 'step');
      expect(breakDancer.currImage).to.be.equal(1);
      clock.tick(timeBetweenSteps); // ? it seems an extra tick is necessary...
      expect(breakDancer.currImage).to.be.equal(2);

      clock.tick(timeBetweenSteps);
      expect(breakDancer.currImage).to.be.equal(3);
    });
  });
});
